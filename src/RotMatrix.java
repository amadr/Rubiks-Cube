...
public class RotMatrix {
	...
	/*
	 * public RotMatrix(double r11, double r12, double r13, double r21, double r22,
	 * double r23, double r31, double r32, double r33) { double matrix[][] = { {
	 * r11, r12, r13 }, { r21, r22, r23 }, { r31, r32, r33 } }; mRotMatrix = matrix;
	 * }
	 */
	...
	public static RotMatrix xRotMatrix(double phi) {
		final double[] xRot = { 1, 0, 0, 0, Math.cos(phi), -Math.sin(phi), 0, Math.sin(phi), Math.cos(phi) };
		final RotMatrix xRotMatrix = new RotMatrix(xRot);
		return xRotMatrix;
	}

	public static RotMatrix yRotMatrix(double phi) {
		final double[] yRot = { Math.cos(phi), 0, Math.sin(phi), 0, 1, 0, -Math.sin(phi), 0, Math.cos(phi) };
		final RotMatrix yRotMatrix = new RotMatrix(yRot);
		return yRotMatrix;
	}

	public static RotMatrix zRotMatrix(double phi) {
		final double[] zRot = { Math.cos(phi), -Math.sin(phi), 0, Math.sin(phi), Math.cos(phi), 0, 0, 0, 1 };
